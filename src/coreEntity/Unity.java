...
public abstract class Unity implements IBaseRavage,ICallBackAStar
{
	...
	// position de l'unitÈ sur une node
	...
	// IntRect du sprite d'animation ‡ afficher
	...
	// enum des modes dans lesquels l'unitÈ peut se trouver
    public static enum ANIMATE {WALK,STRIKE,PAUSE,KILL};
	
    // mode dans lequel l'unitÈ se trouve
	...
	{
		// on vÈrifie si cette enemy existe toujours dans l'entity mananger (si il a n'a pas ÈtÈ tuÈ)
	...
	{
		// emission sur le rÈseau que je suis mort
	...
	protected void NetSend(float x,float y,float dx,float dy,Vec2 vecDir)
	{
			...
			move.setNextPosx(dx);
			move.setNextPosy(dy);
			move.setVecDirFormation(vecDir); // si null alors ce n'est pas encore le message de fin - permet de tourner l'unitÈ vers son sens de formation
	...
	protected void computeRotation(Vec2 vectarget)
	{
		if(vectarget != null)
		{
			...
			float angle = lerp(0.2f,this.body.getAngle(), r.getAngle() ); // angle de la tourelle dÈterminÈ
	...
	public void update(Time deltaTime) 
	{
		...
		if(!nextNode && this.enemyAttribute == null) // si il n'y a pas de nextnode et qu'il n'y a pas d'unitÈ enemy attribuÈe
		{
		...
		// dÈplacement vers un enemy, follow
